...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!proposta_id || !numero_whatsapp)
    ...
    console.log(
      `[WhatsApp API Simulada] Enviando para ${cleanPhone}:`,
      finalMessage,
      pdfUrl ? `[Anexo PDF: ${pdfUrl}]` : '',
    )
    ...
    await supabase
      .from('propostas')
      .update({ status: 'Enviada via WhatsApp' })
      .eq('id', proposta_id)

    return new Response(
      JSON.stringify({ success: true, message_id: 'wa_' + Date.now() }),
      {
