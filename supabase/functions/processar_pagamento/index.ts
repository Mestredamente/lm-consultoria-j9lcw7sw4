...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (plano_id) {
      const planValue = plano_id === 'pro' ? 79.0 : 39.9

      const successUrl = return_url
        ? `${return_url}`
        : `${req.headers.get('origin') || 'http://localhost:5173'}/planos?success=true&plan=${plano_id}`
          ...
          line_items: [
            {
              price_data: {
                currency: 'brl',
                product_data: { name: `Plano ${plano_id.toUpperCase()}` },
                unit_amount: Math.round(planValue * 100),
              },
              quantity: 1,
            },
          ],
      ...
      const mockCheckoutUrl =
        gateway === 'mercado_pago'
          ? 'https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=mock_pix'
          : 'https://pagseguro.uol.com.br/checkout/mock_boleto'
      await new Promise((resolve) => setTimeout(resolve, 800))
      return new Response(
        JSON.stringify({ success: true, checkoutUrl: successUrl }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      )
    }
    ...
    if (!paciente_id || !valor)
      throw new Error('Parâmetros inválidos para pagamento de sessão')
    ...
    const mockCheckoutUrl =
      gateway === 'mercado_pago'
        ...
        : 'https://pagseguro.uol.com.br/checkout/mock'
    await new Promise((resolve) => setTimeout(resolve, 800))
    return new Response(
      JSON.stringify({ success: true, checkoutUrl: mockCheckoutUrl }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
    )
  } catch (error: any) {
